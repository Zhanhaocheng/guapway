...
- 目标线上：<https://guapway-iota.vercel.app>
...
代码先放在 GitHub，再用 Vercel 连仓库。个人博客的 Hobby 套餐足够，不必配环境变量（站点 URL 默认 `https://guapway-iota.vercel.app`）。
...
3. Framework Preset 选 **Next.js**；Project Name 填 **guapway**。`guapway.vercel.app` 已被占用，当前生产域名是 `https://guapway-iota.vercel.app`。
4. 点击 Deploy。之后每次 `main` 的 push 会自动生产部署，Pull Request 会出 Preview URL。

可选：在 Vercel 里加 `NEXT_PUBLIC_SITE_URL=https://guapway-iota.vercel.app`，与默认值一致，方便以后换自定义域名。不要填已失效的 `guapway.vercel.app`。
